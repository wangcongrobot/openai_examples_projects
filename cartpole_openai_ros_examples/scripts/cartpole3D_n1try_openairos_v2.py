...
class DQNRobotSolver():
    def __init__(self, env, environment_name, n_observations, n_actions, n_episodes=1000, n_win_ticks=195, min_episodes=100, max_env_steps=None, gamma=1.0, epsilon=1.0, epsilon_min=0.01, epsilon_log_decay=0.995, alpha=0.01, alpha_decay=0.01, batch_size=64, monitor=False, quiet=False):
        self.memory = deque(maxlen=100000)
        self._env = env

        if monitor:
            self._env = gym.wrappers.Monitor(
                self._env, '../data/cartpole-1', force=True)
    ...
    def choose_action(self, state, epsilon):
        return self._env.action_space.sample() if (np.random.random() <= epsilon) else np.argmax(self.model.predict(state))
